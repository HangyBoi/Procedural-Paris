...
public class PolygonBuildingGeneratorAdj : MonoBehaviour
{
    ...
    void GenerateFacades()
    {
        ...
        for (int i = 0; i < vertexData.Count; i++)
        {
            ...
            if (originalSideDistance < GeometryConstants.GeometricEpsilon) continue;
                                 ...
                                 float pivotOffsetY, float baseLevelY, bool isCornerElement) // isCornerElement not really used here, but kept for signature similarity
    {
        ...
        if (numSegments <= 0 && effectiveSideDistance > GeometryConstants.GeometricEpsilon) numSegments = 1; // Ensure at least one segment if there's some space
    ...
    public float CalculateInteriorCornerAngle(Vector3 pPrev, Vector3 pCurr, Vector3 pNext)
    {
        ...
        if (v1.sqrMagnitude < GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon ||
            v2.sqrMagnitude < GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon)
        {
            return 180f;
        }
        ...
        if (Mathf.Abs(polygonWinding) > GeometryConstants.GeometricEpsilon)
        {
            if (polygonWinding > 0) // CCW polygon
            {
                if (crossProductSign > GeometryConstants.GeometricEpsilon) isConcave = true;
            }
            else // CW polygon
            {
                if (crossProductSign < -GeometryConstants.GeometricEpsilon) isConcave = true;
    ...
    void InstantiateFacadeSegment(List<GameObject> prefabList, Vector3 localPosition, Quaternion localRotation, Transform parent, float segmentWidth, bool isCorner)
    {
        ...
        if (!isCorner && scaleFacadesToFitSide && nominalFacadeWidth > GeometryConstants.GeometricEpsilon && Mathf.Abs(segmentWidth - nominalFacadeWidth) > GeometryConstants.GeometricEpsilon)
        {
    ...
    List<Vector3> CalculateFlatRoofPerimeterVertices(float roofYPosition, float edgeOffset)
    {
        ...
        for (int i = 0; i < vertexData.Count; i++)
        {
            ...
            if (Mathf.Abs(edgeOffset) > GeometryConstants.GeometricEpsilon)
            {
                ...
                else
                {
                    ...
                    if (avgNormal_XZ.sqrMagnitude < GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon)
                    {
    ...
    void CalculateCornerTransform(Vector3 p1_prev, Vector3 p2_current, Vector3 p3_next, out Vector3 cornerPos, out Quaternion cornerRot)
    {
        ...
        if (cornerFacingDirection.sqrMagnitude < GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon)
        {
    ...
    {
        if (nominalFacadeWidth <= GeometryConstants.GeometricEpsilon) return Mathf.Max(1, minSideLengthUnits);
        int num;
    ...
    Vector3 CalculateSideNormal(Vector3 p1, Vector3 p2)
    {
        ...
        if (sideDirection.sqrMagnitude < GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon) return Vector3.forward;
        ...
        if (signedArea > GeometryConstants.GeometricEpsilon) return -initialNormal;
