...
public class CornerGenerator
{
    ...
    public void GenerateAllCorners(Transform cornerRoot)
    {
        ...
        for (int i = 0; i < _vertexData.Count; i++)
        {
            ...
            if (_settings.useMansardFloor && _settings.mansardRiseHeight > GeometryConstants.GeometricEpsilon)
            {
                totalBodySegments += 1;
            }

            // Add a segment for the attic level if it exists and has height
            if (_settings.useAtticFloor && _settings.atticRiseHeight > GeometryConstants.GeometricEpsilon)
            {
                totalBodySegments += 1;
            }
                                          ...
                                          out Vector3 cornerPos_local, out Quaternion cornerRot_local)
    {
        ...
        {
            if (sideNormalNext.sqrMagnitude > GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon)
                cornerFacingDirection = sideNormalNext;
            else if (sideNormalPrev.sqrMagnitude > GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon)
                cornerFacingDirection = sideNormalPrev;
            else
            {
                Vector3 dirIn = (p2_current_local - p1_prev_local).normalized;
                if (dirIn.sqrMagnitude > GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon)
                    cornerFacingDirection = new Vector3(dirIn.z, 0, -dirIn.x).normalized;
                else
                {
                    Vector3 dirOut = (p3_next_local - p2_current_local).normalized;
                    if (dirOut.sqrMagnitude > GeometryConstants.GeometricEpsilon * GeometryConstants.GeometricEpsilon)
